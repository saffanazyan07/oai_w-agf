...
int8_t nr_ue_scheduled_response_stub(nr_scheduled_response_t *scheduled_response) {
  ...
  if (scheduled_response && scheduled_response->ul_config) {
    ...
    fapi_nr_ul_config_request_pdu_t *it = fapiLockIterator(scheduled_response->ul_config);
                                        ...
                                        nr_phy_data_t *phy_data)
{
  ...
  for (int i = 0; i < dl_config->number_pdus; ++i) {
    ...
    // We copy here after the same data in "phy" structures, whatever it is useful ?
    switch (pdu->pdu_type) {
...
{
  fapi_nr_ul_config_request_pdu_t *pdu = fapiLockIterator(ul_config);
  if (!pdu) {
