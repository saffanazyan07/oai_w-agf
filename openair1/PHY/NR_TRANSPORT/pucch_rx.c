                      ...
                      nfapi_nr_pucch_pdu_t* pucch_pdu)
{
  ...
  for (int symb=0; symb<pucch_pdu->nr_of_symbols;symb++) {
    ...
    uint32_t  x2 = (((1<<17)*((14*slot) + (pucch_pdu->start_symbol_index+symb) + 1)*((2*pucch_pdu->dmrs_scrambling_id) + 1)) + (2*pucch_pdu->dmrs_scrambling_id))%(1U<<31); // c_init calculation according to TS38.211 subclause
#ifdef DEBUG_NR_PUCCH_RX
