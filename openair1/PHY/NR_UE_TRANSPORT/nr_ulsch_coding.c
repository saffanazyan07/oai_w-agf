                      ...
                      unsigned int G)
{
  ...
  if (ulsch->pusch_pdu.pusch_data.new_data_indicator) {  // this is a new packet
    ...
    if (A > NR_MAX_PDSCH_TBS) {
      ...
      crc = crc24a(harq_process->aPayload,A)>>8;
      harq_process->aPayload[A>>3] = ((uint8_t*)&crc)[2];
      harq_process->aPayload[1+(A>>3)] = ((uint8_t*)&crc)[1];
      harq_process->aPayload[2 + (A >> 3)] = ((uint8_t *)&crc)[0];
      B = A + 24;
      AssertFatal((A / 8) + 4 <= max_payload_bytes, "A %d is too big (A/8+4 = %d > %d)\n", A, (A / 8) + 4, max_payload_bytes);
      //memcpy(harq_process->b,harq_process->aPayload,(A/8)+4);
    } else {
      // Add 16-bit crc (polynomial A) to payload
      crc = crc16(harq_process->aPayload,A)>>16;
      harq_process->aPayload[A>>3] = ((uint8_t*)&crc)[1];
      harq_process->aPayload[1 + (A >> 3)] = ((uint8_t *)&crc)[0];
      B = A + 16;
      AssertFatal((A / 8) + 3 <= max_payload_bytes, "A %d is too big (A/8+3 = %d > %d)\n", A, (A / 8) + 3, max_payload_bytes);
      //memcpy(harq_process->b,harq_process->aPayload,(A/8)+3);  // using 3 bytes to mimic the case of 24 bit crc
    }
    ...
    impp.Kb = nr_segmentation(harq_process->aPayload,
                              harq_process->c,
                              B,
                              &harq_process->C,
                              &harq_process->K,
