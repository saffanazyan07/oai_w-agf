...
static void nr_postDecode(PHY_VARS_gNB *gNB, notifiedFIFO_elt_t *req, NR_UL_IND_t *UL_INFO)
{
  ...
  if (ulsch_harq->processedSegments == ulsch_harq->C) {
    ...
    if (crc_valid && !check_abort(&ulsch_harq->abort_decode) && !gNB->pusch_vars[rdata->ulsch_id].DTX) {
      ...
      nr_fill_indication(gNB, ulsch->frame, ulsch->slot, rdata->ulsch_id, rdata->harq_pid, 0, 0, UL_INFO);
      ...
      LOG_D(PHY,
            ...
            ulsch_harq->TBS,
            r);
      nr_fill_indication(gNB, ulsch->frame, ulsch->slot, rdata->ulsch_id, rdata->harq_pid, 1, 0, UL_INFO);
      ulsch->handled = 1;
      LOG_D(PHY, "ULSCH %d in error\n",rdata->ulsch_id);
      //      dumpsig=1;
    }
...
static int nr_ulsch_procedures(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, int ULSCH_id, uint8_t harq_pid, NR_UL_IND_t *UL_INFO)
{
  ...
  int nbDecode = nr_ulsch_decoding(gNB,
                                   ULSCH_id,
...
int phy_procedures_gNB_uespec_RX(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, NR_UL_IND_t *UL_INFO)
{
  ...
  for (int ULSCH_id = 0; ULSCH_id < gNB->max_nb_pusch; ULSCH_id++) {
    ...
    if ((ulsch->active == true) && (ulsch->frame == frame_rx) && (ulsch->slot == slot_rx) && (ulsch->handled == 0)) {
      ...
      int const tasks_added = nr_ulsch_procedures(gNB, frame_rx, slot_rx, ULSCH_id, ulsch->harq_pid, UL_INFO);
      if (tasks_added > 0)
