...
void set_tdd_config_nr(nfapi_nr_config_request_scf_t *cfg, frame_structure_t *fs)
{
  ...
  int slot_number = 0;
  int nb_slots_to_set = TDD_CONFIG_NB_FRAMES * fs->numb_slots_frame;
  int slot_index = 0;
  ...
  for (int i = 0; i < nb_slots_to_set; i++) {
    nfapi_nr_max_tdd_periodicity_t *l = &cfg->tdd_table.max_tdd_periodicity_list[i];
    l->max_num_of_symbol_per_slot_list = calloc_or_fail(NR_NUMBER_OF_SYMBOLS_PER_SLOT, sizeof(*l->max_num_of_symbol_per_slot_list));
  }
  ...
  while (slot_number != nb_slots_to_set) {
    ...
    if (tdd_slot_bitmap[slot_index].slot_type == TDD_NR_DOWNLINK_SLOT) {
      for (int number_of_symbol = 0; number_of_symbol < NR_NUMBER_OF_SYMBOLS_PER_SLOT; number_of_symbol++) {
        list->max_num_of_symbol_per_slot_list[number_of_symbol % NR_NUMBER_OF_SYMBOLS_PER_SLOT].slot_config.value = 0;
    ...
    if (tdd_slot_bitmap[slot_index].slot_type == TDD_NR_UPLINK_SLOT) {
      for (int number_of_symbol = 0; number_of_symbol < NR_NUMBER_OF_SYMBOLS_PER_SLOT; number_of_symbol++) {
        list->max_num_of_symbol_per_slot_list[number_of_symbol % NR_NUMBER_OF_SYMBOLS_PER_SLOT].slot_config.value = 1;
        if ((number_of_symbol + 1) % NR_NUMBER_OF_SYMBOLS_PER_SLOT == 0) {
          slot_number++;
          list += 1;
        }
