                        ...
                        time_manager_mode_t running_mode)
{
  ...
  /* hack: #include "openair2/X2AP/x2ap_eNB.h" fails to compile, let's declare
   * is_x2ap_enabled() here, this is all we need
   */
...
printf("mother fucker\n"); fflush(stdout);
  ...
  LOG_I(UTIL, "time manager configuration: [time source: %s] [mode: %s] [server IP: %s} [server port: %d]%s\n",
        (char*[]){ "reatime", "iq_samples" }[time_source_type],
        has_time_server ? "server" : has_time_client ? "client" : "standalone",
        server_ip,
        server_port,
        has_time_server || has_time_client ? "" : " (server IP/port not used)");

  LOG_I(UTIL, "time manager: pdcp tick: %s, rlc tick: %s, x2ap tick: %s\n",
