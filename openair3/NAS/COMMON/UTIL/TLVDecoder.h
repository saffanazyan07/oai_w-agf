...
#ifdef ENABLE_TESTS
#define PRINT_ERROR(...) fprintf(stderr, "TLV Decoder: " __VA_ARGS__)
#else
...
#define CHECK_PDU_POINTER_AND_LENGTH_DECODER(bUFFER, mINIMUMlENGTH, lENGTH)             \
  do {                                                                                  \
    if ((bUFFER) == NULL) {                                                             \
      PRINT_ERROR("(%s:%d) Got NULL pointer for the payload\n", __FILE__, __LINE__);    \
      return TLV_DECODE_BUFFER_NULL;                                                    \
    }                                                                                   \
    if ((lENGTH) < (mINIMUMlENGTH)) {                                                   \
      PRINT_ERROR("(%s:%d) Expecting at least %d bytes, got %u\n", __FILE__, __LINE__, (mINIMUMlENGTH), (lENGTH)); \
...
#define CHECK_LENGTH_DECODER(bUFFERlENGTH, lENGTH)                                      \
  do {                                                                                  \
    if ((bUFFERlENGTH) < (lENGTH)) {                                                    \
      PRINT_ERROR("(%s:%d) Buffer is too short, length (%u) is less than the required length (%u)\n", \
                  __FILE__,                                                             \
                  __LINE__,                                                             \
                  (bUFFERlENGTH),                                                       \
                  (lENGTH));                                                            \
...
#define CHECK_MESSAGE_TYPE(mESSAGE_TYPE, BUFFER)                                                                        \
  do {                                                                                                                  \
    if ((MESSAGE_TYPE) != (BUFFER)) {                                                                                   \
      PRINT_ERROR("(%s:%d) Wrong message type: Got: %u, Expected: %u\n", __FILE__, __LINE__, (BUFFER), (MESSAGE_TYPE)); \
