...
void RCconfig_NR_L1(void)
{
  ...
  if (L1_ParamList.numelt > 0) {
    for (j = 0; j < RC.nb_nr_L1_inst; j++) {
      ...
      RC.gNB[j]->tx_amp_backoff_dB = *L1_ParamList.paramarray[j][L1_TX_AMP_BACKOFF_dB].uptr;
      RC.gNB[j]->rx_power_reference_dBm = *L1_ParamList.paramarray[j][L1_RX_POWER_REFERENCE_DBM].iptr;
