...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP, frame_t frame, sub_frame_t slot, NR_Sched_Rsp_t *sched_info)
{
  ...
  if (get_softmodem_params()->phy_test == 0) {
    ...
    // get prach duration in slots from rach_ConfigCommon
    ...
    const int max_prach_len = (prach_fmt < 4) ? get_long_prach_dur(prach_fmt, *scc->ssbSubcarrierSpacing) : 1;
    const sub_frame_t n_slots_ahead = nr_slots_per_frame[*scc->ssbSubcarrierSpacing] - max_prach_len + get_NTN_Koffset(scc);
