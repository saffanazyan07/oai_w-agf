...
static void nr_rrc_ue_generate_rrcReestablishmentComplete(const NR_UE_RRC_INST_t *rrc, const NR_RRCReestablishment_t *rrcReestablishment, RrcDcchDataResp *returned);
...
static void nr_rrc_ue_process_ueCapabilityEnquiry(NR_UE_RRC_INST_t *rrc, NR_UECapabilityEnquiry_t *UECapabilityEnquiry, RrcDcchDataResp *returned);
                                                  ...
                                                  RrcDcchDataResp *returned)
{
  ...
  returned->security_mode = ue_rrc->integrityProtAlgorithm << 4;
  ...
  if (!integrity_pass) {
    ...
    returned->doCypheringKeys = true;
    returned->security_mode = ue_rrc->cipheringAlgorithm | (ue_rrc->integrityProtAlgorithm << 4);
  }
  int maxSz, *setSz;
...
static void nr_rrc_ue_generate_RRCReconfigurationComplete(NR_UE_RRC_INST_t *rrc, const int srb_id, const uint8_t Transaction_id,                                                   RrcDcchDataResp *returned)
{
  int maxSz, *setSz;
  char *buffer = getRetBuf(returned, &maxSz, &setSz);
...
static void nr_rrc_ue_process_rrcReestablishment(NR_UE_RRC_INST_t *rrc,
                                                 const int gNB_index,
                                                 const NR_RRCReestablishment_t *rrcReestablishment, RrcDcchDataResp *returned)
{
                                         ...
                                         const bool integrity_pass)
{
  ...
  *returned = (RrcDcchDataResp){.doCypheringKeys = false};
    ...
    case NR_DL_DCCH_MessageType_PR_c1: {
      ...
      switch (c1->present) {
        ...
        case NR_DL_DCCH_MessageType__c1_PR_rrcReconfiguration: {
          ...
          nr_rrc_ue_generate_RRCReconfigurationComplete(rrc, Srb_id, c1->choice.rrcReconfiguration->rrc_TransactionIdentifier, returned);
        } break;

        case NR_DL_DCCH_MessageType__c1_PR_rrcResume:
          LOG_I(NR_RRC, "Received rrcResume on DL-DCCH-Message, need to develop processing and answer\n");
          break;
